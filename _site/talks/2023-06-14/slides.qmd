...
-   Exclude those with any missing items (complete-case)

-   Change denominator to number of non-missing items (proration; i.e. person-mean imputation)

-   Multiple imputation (of total score, of individual items)
...
-   Throwing away *a lot* of data, including from people who have almost all items observed
-   Valid only under the strongest set of assumptions

Proration

-   Since the items cover a range of domains, this can mean that single domains e.g., cognition and mental health, are weighted differently than others
-   Still throwing away data from observations not meeting the threshhold of enough items

Multiple imputation

-   Computationally intensive
-   Not valid in general under MNAR
...
| missingness does not depend on the observed or missing data | conditional on the observed data, missingness does not depend on the missing data | missingness depends on the missing data, even conditional on the observed data |
...
Under MAR, $\color{IndianRed}{f(Y_{mis} \mid Y_{obs}, R, X)} = f(Y_{mis} \mid Y_{obs}, X)$: missing data doesn't depend on missingness pattern

## Pattern-mixture models

Model how the distribution of missing data depends on missingness pattern

-   For example, a missingness pattern in which a given variable $Y$ is missing may be associated with a *higher* probability that $Y = 1$

-   We would never know that from the observed data, because by definition we are missing $Y$ in that missingness pattern

-   But in a sensitivity analysis, we can decide how much higher it might be
...
For a single variable with missingness:

$$
E[Y \mid R, X] = \beta_0 + \beta_1X + \color{IndianRed}{\delta} I(R = \color{SlateBlue}{r_0})
$$

where $\color{IndianRed}{\delta}$ parameterizes how much different the distribution of $Y$ is in observations with missing data patterns where it is missing ($\color{SlateBlue}{r_0}$)

## Multiple imputation

The delta adjustment approach can be done in the context of multiple imputation, e.g., with MICE

-   Fit a model for $Y$ as usual
-   Add $\delta$ to the imputed values
...
-   @tompsett2018use proposed a solution which involves eliciting mode interpretable delta-like parameters and searching the solution space for the $\delta$s they correspond to
...
::: center
![**Difficulty bathing**](img/bathing.png)
:::
...
#| message: false
library(ggokabeito)
example_plots <- read_rds("~/Documents/Projects/All of Us/missing-data/_targets/objects/example_plots")
example_plots$rank_plot +
    labs(y = NULL, x = "Scaled ranks of sensitivity parameters") + scale_fill_manual(values = c("SlateBlue", "#008080", "IndianRed", "Gold")) +
